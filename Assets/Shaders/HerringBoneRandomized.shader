Shader "Unlit/HerringBoneRandomized"
{
	//This uses a random function to choose which tile to show, instead of basing it on another texture
	Properties
	{
	...
	SubShader
	{
		...
		Pass
		{
			...
			fixed4 frag (v2f i) : SV_Target
			{
				...
				if ((uv.x + uv.y) % 2 == 1) {
					if (floor(uv.x - uv.y+_TextureSize) % 4 == 1) {						
						uv = uv + fixed2(-1, 0);
					...
					if (floor(uv.x - uv.y + _TextureSize) % 4 == 0) {
